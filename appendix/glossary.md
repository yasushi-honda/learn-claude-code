...
### Channels
実行中の Claude Code セッションに外部イベント（チャットメッセージ、Webhook、アラート）を Push する MCP サーバー。Telegram / Discord / iMessage のプリビルトプラグインが利用可能。`--channels` フラグでセッション起動時に有効化する。Research Preview 機能（v2.1.80+）。

### Channels Reference
カスタムチャネルを構築するための技術仕様。`claude/channel` ケイパビリティの宣言、`notifications/claude/channel` イベントの発行、返信ツールの公開、送信者ゲーティング、パーミッションリレーの実装方法を定義する。

### スケジュールタスク（Scheduled Tasks）
プロンプトを定期的に自動実行する機能。3 種類あり：(1) `/loop` はセッション内のクイックポーリング（セッションスコープ、3 日で期限切れ）、(2) Desktop スケジュールはローカルマシンで永続実行、(3) Cloud スケジュールは Anthropic インフラ上で実行（マシン電源不要）。

### Voice Dictation
push-to-talk 方式の音声入力機能。`/voice` コマンドで有効化し、Space キーを長押しして発話するとリアルタイムで文字起こしされる。20 言語対応。claude.ai アカウント認証が必要。

### Code Review
GitHub PR をマルチエージェントで自動分析し、インラインコメントとして問題を報告するマネージドサービス。重要度を Normal（🔴）/ Nit（🟡）/ Pre-existing（🟣）の 3 段階で分類。`REVIEW.md` と `CLAUDE.md` でカスタマイズ可能。Teams / Enterprise 向け Research Preview。
